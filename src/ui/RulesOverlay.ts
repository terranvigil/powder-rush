...
export function showRules(): void {
  ...
  const kLeft = keyBindings.displayName(keyBindings.get("steerLeft"));
  const kRight = keyBindings.displayName(keyBindings.get("steerRight"));
  const kTuck = keyBindings.displayName(keyBindings.get("tuck"));
  const kBrake = keyBindings.displayName(keyBindings.get("brake"));
  ...
  panel.innerHTML = `
    ...
    <div class="rules-body">
      ...
      <p>Hold <span class="rules-hl">${kTuck}</span> to go faster. What happens depends on your speed:</p>
      ...
      <p><span class="rules-hl">Spin</span> &mdash; Steer (${kLeft}/${kRight}) while airborne</p>
      <p><span class="rules-hl">Grab</span> &mdash; Tuck (${kTuck}) while airborne</p>
      <p><span class="rules-hl">Flip</span> &mdash; Brake (${kBrake}) while airborne</p>
